...
class RGBMatrix : public Canvas {
  public:
    ...
    static std::unique_ptr<RGBMatrix> CreateFromOptions(const Options& options, const RuntimeOptions& runtime_options);
    ...
    // Returns NULL, if there was a problem (a message then is written to stderr).
    static std::unique_ptr<RGBMatrix> CreateFromFlags(int* argc,
                                                      char*** argv,
                                                      RGBMatrix::Options* default_options = nullptr,
                                                      RuntimeOptions* default_runtime_opts = nullptr,
    ...
    //
    // Note: The frame canvas is owned as a unique_ptr inside the matrix and the
    //       the raw pointer returned is therefore non-owning
    FrameCanvas* const CreateFrameCanvas();
    ...
    FrameCanvas* const SwapOnVSync(FrameCanvas* other, unsigned framerate_fraction = 1);
    ...
    RGBMatrix* gpio() __attribute__((deprecated)) {
        return this;
    }
  ...
  private:
    ...
    std::vector<std::unique_ptr<FrameCanvas>> created_frames_;
...
bool ParseOptionsFromFlags(int* argc, char*** argv, RGBMatrix::Options* default_options, RuntimeOptions* rt_options, bool remove_consumed_flags = true);

// Show all the available options in a style that can be used in a --help
// output on the command line.
void PrintMatrixFlags(FILE* out, const RGBMatrix::Options& defaults = RGBMatrix::Options(), const RuntimeOptions& rt_opt = RuntimeOptions());
