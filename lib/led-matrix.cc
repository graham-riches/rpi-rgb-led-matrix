...
class RGBMatrix::Impl::UpdateThread : public Thread {
  public:
    UpdateThread(GPIO* io, FrameCanvas* initial_frame, int pwm_dither_bits, bool show_refresh, int limit_refresh_hz)
        : io_(io)
        , show_refresh_(show_refresh)
        , target_frame_usec_(limit_refresh_hz < 1 ? 0 : 1e6 / limit_refresh_hz)
        , running_(true)
        , current_frame_(initial_frame)
        , next_frame_(NULL)
    ...
    virtual void Run() {
        ...
        while ( running() ) {
            ...
            current_frame_->framebuffer()->DumpToMatrix(io_, start_bit_[low_bit_sequence % 4]);
...
void RGBMatrix::Impl::SetGPIO(GPIO* io, bool start_thread) {
    if ( io != NULL && io_ == NULL ) {
        ...
        Framebuffer::InitGPIO(io_,
                              ...
                              params_.pwm_dither_bits,
                              params_.row_address_type);
        Framebuffer::InitializePanels(io_, params_.panel_type, params_.cols * params_.chain_length);
    }
    if ( start_thread ) {
        StartRefresh();
    }
...
uint8_t RGBMatrix::Impl::pwmbits() {
    return params_.pwm_bits;
}
...
/**
 * \brief factory method for constructing a new RGBMatrix object
 * 
 * \param options Options struct to configure the matrix's physical properties
 * \param runtime_options runtime options to configure the runtime behavior
 * \retval std::unique_ptr<RGBMatrix> returns a new object to the caller with ownership
 */
std::unique_ptr<RGBMatrix> RGBMatrix::CreateFromOptions(const RGBMatrix::Options& options, const RuntimeOptions& runtime_options) {
    std::string error;
...
uint64_t RGBMatrix::RequestInputs(uint64_t all_interested_bits) {
    return gpio_ptr->RequestInputs(all_interested_bits);    
}
...
uint64_t RGBMatrix::RequestOutputs(uint64_t all_interested_bits) {
    uint64_t success_bits = gpio_ptr->InitOutputs(all_interested_bits);
